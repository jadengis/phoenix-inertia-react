...
RUN apt update \
  && apt install -y --no-install-recommends libstdc++6 openssl libncurses5 locales ca-certificates nodejs \
  && rm -rf /var/lib/apt/lists/*
...
# advised to add an init process such as tini via `apt install`
